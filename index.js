...
module.exports = function TRNM(d) {
	...
	d.command.add('trninfo', (arg) => {
		d.command.message('enabled: ' + `${enabled}`.clr('00FFFF'));
		d.command.message('insidemap: ' + insidemaa);
	...
	function sLoadTopo(event) {
		if (event.zone === mapID[0]) {								
			insidemap = true;
			d.command.message('Welcome to ' + 'TR'.clr('56B4E9') + '[NM]'.clr('E69F00'));
			load();
		} else if (event.zone === mapID[1]) {
			insidemap = true;
			d.command.message('Welcome to ' + 'TR'.clr('56B4E9') + '[HM]'.clr('00FFFF'));
			load();
		} else {
			unload();
		}
